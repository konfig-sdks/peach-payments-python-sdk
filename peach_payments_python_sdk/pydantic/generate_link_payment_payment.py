...
from peach_payments_python_sdk.pydantic.generate_link_payment_payment_files import GenerateLinkPaymentPaymentFiles

class GenerateLinkPaymentPayment(BaseModel):
    # Invoice ID that can be used to link the payment to an invoice on the merchant's system. Must be less than 17 characters.
    merchant_invoice_id: str = Field(alias='merchantInvoiceId')
    ...
    currency: Literal["ZAR", "KES", "USD"] = Field(alias='currency')

    files: typing.Optional[GenerateLinkPaymentPaymentFiles] = Field(None, alias='files')
